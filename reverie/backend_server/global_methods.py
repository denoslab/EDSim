...
def copyanything(src, dst):
  ...
  if os.path.isdir(src):
    ...
    create_folder_if_not_there(dst)
    shutil.copy2(src, dst)

def read_csv_to_dict(file):
  data = numpy.loadtxt(file, delimiter=',', dtype=str)
  labels = data[0]
  data_dict = dict()
  ...
  for i in range(len(labels)):
    ...
    for j in range(1, len(data)):
      data_dict[labels[i]].append(data[j,i])
  return data_dict

def array_to_2d(tile_layer, height, width):
  layer_data = []
  for i in range(height):
      offset = i*width
      layer_data.append(tile_layer["data"][(offset):(width + offset)])
