...
class Scratch: 
  def __init__(self, f_saved): 
    ...
    self.schedule_constraints = None
    ...
    self.behavior_modifiers = None
    # L1 stable traits.
    self.role_description = None
    # L2 external implementation. 
    self.current_state = None
    self.work_pattern = None
    ...
    # <event_form> represents the event triple that the persona is current_state 
    # engaged in. 
    ...
    # Control the movements of the persona by assigning this an action
    self.next_step = None
    ...
    if check_if_file_exists(f_saved): 
      ...
      self.schedule_constraints = scratch_load["schedule_constraints"]
      ...
      self.behavior_modifiers = scratch_load["behavior_modifiers"]
      self.role_description = scratch_load["role_description"]
      self.current_state = scratch_load["current_state"]
      self.work_pattern = scratch_load["work_pattern"]
      ...
      pron = scratch_load.get("act_pronunciatio")
      if pron is None:
        pron = scratch_load.get("act_pronunciation")
      self.act_pronunciatio = pron
      self.act_event = tuple(scratch_load["act_event"])

      self.act_obj_description = scratch_load["act_obj_description"]

      pron = scratch_load.get("act_obj_pronunciatio")
      if pron is None:
        pron = scratch_load.get("act_obj_pronunciation")
      self.act_obj_pronunciatio = pron
      ...
      # Robustness: older saves may store this as a string; always keep a dict.
      if not isinstance(self.chatting_with_buffer, dict):
      ...
      self.next_step = scratch_load["next_step"]
  ...
  def save(self, out_json):
    ...
    scratch["schedule_constraints"] = self.schedule_constraints

    scratch["name"] = self.name
    scratch["first_name"] = self.first_name
    scratch["last_name"] = self.last_name
    scratch["age"] = self.age
    scratch["behavior_modifiers"] = self.behavior_modifiers
    scratch["role_description"] = self.role_description
    scratch["current_state"] = self.current_state
    scratch["work_pattern"] = self.work_pattern
    ...
    scratch["next_step"] = self.next_step
  ...
  def get_str_iss(self): 
    ...
    commonset += f"Innate traits: {self.behavior_modifiers}\n"
    commonset += f"Learned traits: {self.role_description}\n"
    commonset += f"Currently: {self.current_state}\n"
    commonset += f"Lifestyle: {self.work_pattern}\n"
    commonset += f"Daily plan requirement: {self.schedule_constraints}\n"
    if(self.curr_time):
  ...
  def get_str_innate(self): 
    return self.behavior_modifiers
