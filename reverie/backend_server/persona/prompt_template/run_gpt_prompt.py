...
def run_gpt_generate_patient_scratch(role,patient_description, verbose = False):
  ...
  def __offline_fallback():
    ...
    """
    # Try to extract the name from the prompt; fall back to a generic role-based name.
    name_match = re.search(r"Name:\\s*\\(([^)]+)\\)", patient_description or "")
    extracted_name = name_match.group(1) if name_match else f"{role} {get_random_alphanumeric(3, 4)}"
